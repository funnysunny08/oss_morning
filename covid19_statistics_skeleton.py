...
# ratio --> result = n_covid[0]/sum_covid*100
...
print('| ------ | --------- | --------- | -------------- |')
for idx, pop in enumerate(n_people):
    ratio2 = n_covid[idx]/sum_covid*100 
    print('| %s | %d | %.1f | %.1f |' % (regions[idx], n_covid[idx], ratio2, normalize_data(n_covid, n_people, 1000000)[idx]))
print('')